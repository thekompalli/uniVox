...
class ResultRepository:
    ...
    async def initialize(self):
        ...
        If DB is unavailable, fall back to file-only mode without raising.
        """
        try:
            async with self._lock:
                if self.pool is None:
                    try:
                        ...
                        logger.info("Result repository initialized (DB mode)")
                    except Exception as db_err:
                        self.pool = None
                        logger.warning(f"Result repository DB init failed, using file-only mode: {db_err}")
                        logger.info("Result repository initialized (file-only mode)")
    ...
    async def save_result(
        ...
        """
        try:
            # Save to database when available; otherwise file-only
            ...
            # Always save to file system for backup / fallback
            await self._save_result_to_file(job_id, result_type, result_data, file_paths)
            logger.info(f"Saved {result_type} result for job {job_id}{' (file-only)' if self.pool is None else ''}")
            return True
    ...
    async def get_result(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self.pool is None:
                # DB not available in this process; load from file system
    ...
    async def save_validation_results(
        ...
        try:
            if self.pool is None:
                # Persist to file and succeed silently
                await self._save_validation_to_file(job_id, validation_data)
                logger.info(f"Saved validation results for job {job_id} (file-only)")
                return True
            async with self.pool.acquire() as connection:
                await connection.execute("""
    ...
    async def get_validation_results(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self.pool is None:
                return await self._load_validation_from_file(job_id)
            async with self.pool.acquire() as connection:
                row = await connection.fetchrow("""
    ...
    async def save_performance_metrics(
        ...
        try:
            if self.pool is None:
                await self._save_metrics_to_file(job_id, metrics)
                logger.info(f"Saved performance metrics for job {job_id} (file-only)")
                return True
            async with self.pool.acquire() as connection:
                await connection.execute("""
                    INSERT INTO performance_metrics (
    ...
    async def get_performance_metrics(self, job_id: str) -> Optional[Dict[str, float]]:
        ...
        try:
            if self.pool is None:
                return await self._load_metrics_from_file(job_id)
            async with self.pool.acquire() as connection:
                row = await connection.fetchrow("""
                    SELECT * FROM performance_metrics WHERE job_id = $1
                """, job_id)
    ...
    async def _save_validation_to_file(self, job_id: str, validation_data: Dict[str, Any]):
        try:
            job_dir = self.file_storage_path / job_id
            job_dir.mkdir(exist_ok=True)
            with open(job_dir / "validation_results.json", 'w', encoding='utf-8') as f:
                json.dump(validation_data, f, indent=2, default=str)
        except Exception as e:
            logger.warning(f"Failed to write validation fallback for {job_id}: {e}")

    async def _load_validation_from_file(self, job_id: str) -> Optional[Dict[str, Any]]:
        try:
            fp = self.file_storage_path / job_id / "validation_results.json"
            if not fp.exists():
                return None
            with open(fp, 'r', encoding='utf-8') as f:
                data = json.load(f)
            all_valid = all([
                data.get('sid_csv', {}).get('valid', False),
                data.get('sd_csv', {}).get('valid', False),
                data.get('lid_csv', {}).get('valid', False),
                data.get('asr_trn', {}).get('valid', False),
                data.get('nmt_txt', {}).get('valid', False),
            ])
            return {**data, 'all_valid': all_valid}
        except Exception as e:
            logger.warning(f"Failed to read validation fallback for {job_id}: {e}")
            return None

    async def _save_metrics_to_file(self, job_id: str, metrics: Dict[str, Any]):
        try:
            job_dir = self.file_storage_path / job_id
            job_dir.mkdir(exist_ok=True)
            ...
            logger.warning(f"Failed to write metrics fallback for {job_id}: {e}")

    async def _load_metrics_from_file(self, job_id: str) -> Optional[Dict[str, Any]]:
        try:
            fp = self.file_storage_path / job_id / "metrics.json"
            if not fp.exists():
                return None
            with open(fp, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception as e:
            logger.warning(f"Failed to read metrics fallback for {job_id}: {e}")
            return None
    ...
    async def _save_result_to_file(
        ...
        try:
            ...
            with open(result_file, 'w', encoding='utf-8') as f:
                json.dump(result_data, f, default=str, indent=2, ensure_ascii=False)
    ...
    def save_result_sync(self, job_id: str, result_data: Dict[str, Any]) -> bool:
        """Save result synchronously to both database and file"""
        try:
            import asyncio
            ...
            # Also save to database using async method
            try:
                # Run async save_result in sync context
                ...
                try:
                    loop.run_until_complete(
                        self.save_result(
                            ...
                            result_data=result_data,
                            file_paths=result_data.get('output_files', {}),
                            metadata={"saved_sync": True}
                        )
                    )
                    logger.info(f"Saved result to database for job {job_id}")
                finally:
                    loop.close()
                    
            except Exception as db_error:
                logger.warning(f"Failed to save to database for job {job_id}: {db_error}")
